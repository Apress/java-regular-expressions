...
* white space character, not \\s. Accordingly, this class does not allow
* the regex patterns to be modified programmatically, nor does it
* follow the normal property file convention for \n,\t, etc., or
* multi line properties. Please see the documentation for the
* load method
*/
public class RegexProperties extends Properties{
    ...
    *
    A line that contains only white space or whose first
   ...
   public void load(FileInputStream inStream) throws IOException, PatternSyntaxException{
    ...
    while (matcher.find()){
        ...
        )
        {
            ...
            //was the key- value entry split with the '='
            //character or the ':' character? Both are legal.
            if (line.indexOf("=") > 0  )
              keyValue = line.split("=",2);//only consume first =
            else
              keyValue = line.split(":",2);//only consume first :
